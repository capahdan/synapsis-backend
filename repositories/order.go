...
func (r *orderRepository) GetAllOrders(page, limit int, status string) ([]models.Order, int, error) {
	var (
		orders []models.Order
		count  int64
	)
	...
	if status != "" {
		err = r.db.Where("status = ?", status).Limit(limit).Offset(offset).Find(&orders).Count(&count).Error
		return orders, int(count), err
	}
